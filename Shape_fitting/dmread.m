...
fid = fopen(filename,'r','l'); %opens file
if fid == -1
    error('Could not open file.');
end
c = onCleanup(@()fclose(fid)); %when c is destroyed,the file is closed
version = fread(fid,1,'*int32',0,'b'); %reads data with all these specs
%create root tags structure %creates the structure that 
tags = struct();
if version == 3
    fseek(fid,4,'cof');
elseif version == 4
